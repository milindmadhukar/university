...
def hash_message(message: str) -> str:
    ...
    original_bit_length = len(message_array) * 8
    
    # Add padding byte
    message_array.append(0x80)
    
    # Pad with zeros until we reach the right length
    while (len(message_array) + 16) % 128 != 0:
        message_array.append(0x00)
        
    # Append the original message length as a 128-bit big-endian number
    message_array.extend(struct.pack('>Q', 0))  # First 64 bits are always 0 for short messages
    message_array.extend(struct.pack('>Q', original_bit_length))  # Last 64 bits are the length
    ...
    for chunk_start in range(0, len(message_array), 128):
        ...
        # 80 words of 64 bits each
        w = [0] * 80

        # First 16 words are the message
        w[:16] = struct.unpack("!16Q", chunk)

        # Remaining 64 words are calculated
...
if __name__ == "__main__":
    inp = input("Enter the message to hash: ")
    print(f"SHA-512 hash of {inp}: {hash_message(inp)}")
